...
### 필수: 세션 시작 시 대시보드 우선 읽기
**토큰 절약을 위해 `PROJECT-STATUS.md`를 가장 먼저 읽습니다.**
...
#### 업데이트 시점 (하이브리드)
1. **작업 완료 시**: 에이전트가 직접 대시보드 업데이트
2. **수동 명령**: `상태 업데이트` 또는 `대시보드 갱신`

#### 명령어
- `상태 업데이트` / `대시보드 갱신`: PROJECT-STATUS.md 수동 업데이트
- `다음 할 일`: 대시보드의 다음 할 일 목록 표시

#### 대시보드 위치
- `PROJECT-STATUS.md` (프로젝트 루트)
